...
const store = createStore({
    ...
    actions:{
        async get_wells({commit}, payload){
            ...
            const response = await axios.get('http://0.0.0.0:5000/lookup_wells', {responseType:'json','params':{
                'apis': payload.apis,
        ...
        async load_assets({commit},payload){
            const response = await axios.get('http://0.0.0.0:5000/get_assets', {responseType:'json',headers:{'Content-Type':'application/json'},'params':{
